...
class SafeSectionConvenienceWidget extends StatefulWidget {
  const SafeSectionConvenienceWidget({super.key, required this.convenienceController, required this.gymController, required this.maxDisplay});
  final ConvenienceController convenienceController;
  final GymController gymController;
  final int maxDisplay;

  @override
  State<SafeSectionConvenienceWidget> createState() => _SafeSectionConvenienceWidgetState();
}

class _SafeSectionConvenienceWidgetState extends State<SafeSectionConvenienceWidget> {
    ...
    return Column(
      children: [
        Row(
          ...
          children: [
            Text('An ninh', style: fontRegular.copyWith(fontSize: 16, fontWeight: FontWeight.w500)),
            if (widget.convenienceController.safeConveniences.length > widget.maxDisplay)
              TextButton(
                onPressed: () {
        ...
        GridView.builder(
          ...
          itemCount: widget.convenienceController.safeConveniences.length > widget.maxDisplay
              ? _isExpanded
                    ? widget.convenienceController.safeConveniences.length
                    : widget.maxDisplay
              : widget.convenienceController.safeConveniences.length, // Total number of items
          itemBuilder: (context, index) {
            return GestureDetector(
              onTap: () {
                List<String> favoriteConvenience = widget.gymController.gymFilter.favoriteConvenience;
                String value = widget.convenienceController.safeConveniences[index].id!;
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: widget.gymController.gymFilter.favoriteConvenience.contains(widget.convenienceController.safeConveniences[index].id!)
                      ? AppColors.orange300
                      : Colors.grey[200], // Màu nền thay đổi khi được chọn
                ),
                child: Text(
                  widget.convenienceController.safeConveniences[index].name!,
                  style: TextStyle(
                    color: widget.gymController.gymFilter.favoriteConvenience.contains(widget.convenienceController.safeConveniences[index].id!)
                        ? Colors.white
